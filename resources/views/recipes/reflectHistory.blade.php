                ...
                <div class="gap-1 columns-2">
                    ...
                    <div id="popup" class="hidden absolute top-0 left-0 mt-8 p-4 bg-white border border-gray-300 rounded shadow-lg">
                        <p>アイコンの説明<br>
                            ...
                            <div class="flex">
                                <img src="{{ asset('images/index/history.svg') }}" alt="変更履歴から手順を引用する">
                                <p>： 手順の変更履歴から、希望の手順を引用します。</p>
                            </div>
                            <div class="flex">
                ...
                @foreach($recipe['steps'] as $i => $os)
                    @if ($os['created_at'] == $recipe['steps'][(count($recipe['steps'])-1)]['created_at'])
                        <div class="step flex my-3">
                            ...
                            <img class="handle mr-3 custom-cursor" src="{{ asset('images/index/swapVert.svg') }}" alt="順序を入れ替える">
                            <div class="relative m-3">
                                @if ($os['id'] == $step_id)
                                    <textarea type="text" id="step_create" name="steps[]" class="step-input block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-white rounded-lg border border-gray-300 appearance-none dark:text-white dark:bg-gray-700 dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" rows="4" cols="50" placeholder="">{{$posts['reflectDescription']}}</textarea>
                            ...
                            <div class="mt-6">
                                <a href="{{ route('recipe.changeHistory',  ['recipe' => $recipe['id'], 'step' => $os['id']]) }}"><img src="{{ asset('images/index/history.svg') }}" alt="変更履歴" class="mb-9 custom-cursor"></a>
