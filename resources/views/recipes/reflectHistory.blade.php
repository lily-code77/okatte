    ...
    <form action="{{ route('recipe.update',  ['recipe' => $recipe['id']]) }}" method="post" enctype="multipart/form-data">
        ...
        <div class="flex justify-evenly">
            ...
            <div class="py-20 px-20">
                <a href="{{ route('mindmap.show', ['recipe' => $recipe['id']]) }}" target="_blank"><button type="button" class="mindmap-btn rounded text-sm">Mind Map</button></a>
            </div>
        </div>
            ...
            @foreach($recipe['steps'] as $i => $os)
                @if ($os['created_at'] == $recipe['steps'][(count($recipe['steps'])-1)]['created_at'])
                    <div class="step flex my-3">
                        ...
                        <div class="relative m-3">
                            @if ($os['id'] == $step_id)
                                <textarea type="text" id="step_create" name="steps[]" class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-white rounded-lg border-1 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" rows="4" cols="50" placeholder="">{{$posts['reflectDescription']}}</textarea>
